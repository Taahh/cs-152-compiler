...
fn main() {
    ...
    fn parse_expr(tokens: &Vec<String>, i: &mut usize)
    {
        if matches!(parse(tokens[*i].clone()), Token::Int) {
            ...
            *i += 1;
            ...
            // if  matches!(parse(tokens[*i].clone()), Token::OpeningBracket) {
            //     eprintln!("Error : Expected an identifier for the variable declaration, or an array size given such as [1], or [4]");
            //     process::exit(1);
            // }
            // println!("{}, {}", *i, tokens[*i].clone());
            //
            // // if matches!(parse(tokens[*i].clone()), Token::OpeningBracket) {
            // //     *i += 1;
            // //     if !matches!(parse(tokens[*i].clone()), Token::Num(i32)) {
            // //         eprintln!("Error : Expected a numerical integer size to be given");
            // //         process::exit(1);
            // //     }
            // //     println!("{}, {}", *i, tokens[*i].clone());
            // //     *i += 1;
            // //     if !matches!(parse(tokens[*i].clone()), Token::ClosingBracket) {
            // //         eprintln!("Error : Expected a closing bracket for array size initialization.");
            // //         process::exit(1);
            // //     }
            // //     println!("{}, {}", *i, tokens[*i].clone());
            // // } else {
        ...
        } else if matches!(parse(tokens[*i].clone()), Token::If) {
            println!("{}, {}", *i, tokens[*i].clone());
            *i += 1;
    ...
    fn parse_body(tokens: &Vec<String>, balanced_braces: &mut Vec<Token>, i: &mut usize) {
        if matches!(parse(tokens[*i].clone()), Token::While)
        {
            ...
            *i += 1;
            ...
            while !matches!(parse(tokens[*i].clone()), Token::ClosingBrace) {
                if matches!(parse(tokens[*i].clone()), Token::ClosingBrace) {
                    balanced_braces.pop();
                    println!("{}, {}", i, tokens[*i].clone());
                    break;
                }
                // parse expression
                parse_body(tokens, balanced_braces, i);
            }
        } else {
            parse_expr(tokens, i);
        }
    ...
    {
        let mut balanced_braces: Vec<Token> = vec![];
        let mut i: usize = 0;
        while i < tokens.len()
        {
                ...
                Token::Func =>
                    {
                        ...
                        while i < tokens.len() - 1 {
                            // parse expression
                            parse_body(&tokens, &mut balanced_braces, &mut i);
                        }
                        balanced_braces.pop();
                        println!("{}, {}", i, tokens[i].clone());
                        // println!("{}", balanced_braces.len());
                    }
                _ => { i += 1; }
            }
        }
